'''
1. Perform selective search on the composite images
2. Filter the regions proposed into valid regions
3. Used publicly available faster version of Non-maximal supression (Malisiewicz et al.)
'''
import numpy as np
import cv2
import selectivesearch as ss
import A_B_fastnms as nms
...
def getRegionProposals(path, imageList, size):
    ...
    for image in imageList:
        ...
        print noBBeach
        print np.array(regionProposedeach).shape
        #raw_input("YYYYY:")


        ### ----------- EACH IMAGE NMS -------------- ###
        regionProposedeach = nms.nonMaximumSuppression(np.array(regionProposedeach,dtype="int32"),0.5)
        '''print regionProposedeach
        print regionProposedeach.dtype
        print regionProposedeach.shape
        '''
        noBBeach = regionProposedeach.shape[0]
        
        regionProposedeach= regionProposedeach.tolist()
        img2 = img.copy()
        for bb in regionProposedeach:
            cv2.rectangle(img2,(int(bb[0]),int(bb[1])),(int(bb[2]),int(bb[3])),(0,255,0),1)

        #draw rectangles on original image
        cv2.imwrite('nms/'+image,img2)
